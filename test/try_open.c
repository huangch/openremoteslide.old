...
static void check_error(openremoteslide_t *osr) {
  const char *error = openremoteslide_get_error(osr);
  if (error != NULL) {
    fail("%s", error);
  }
}
...
static void check_api_failures(openremoteslide_t *osr) {
  int64_t w, h;

  CHECK_RET(openremoteslide_get_level_count(osr), -1);
  CHECK_W_H(openremoteslide_get_level0_dimensions(osr, &w, &h), -1, -1);
  CHECK_W_H(openremoteslide_get_level_dimensions(osr, 0, &w, &h), -1, -1);
  CHECK_W_H(openremoteslide_get_level_dimensions(osr, 27, &w, &h), -1, -1);
  CHECK_W_H(openremoteslide_get_level_dimensions(osr, -3, &w, &h), -1, -1);
  CHECK_RET(openremoteslide_get_level_downsample(osr, 0), -1);
  CHECK_RET(openremoteslide_get_level_downsample(osr, 27), -1);
  CHECK_RET(openremoteslide_get_level_downsample(osr, -3), -1);
  CHECK_RET(openremoteslide_get_best_level_for_downsample(osr, 0.8), -1);
  CHECK_RET(openremoteslide_get_best_level_for_downsample(osr, 2), -1);
  CHECK_RET(openremoteslide_get_best_level_for_downsample(osr, 4096), -1);
  CHECK_EMPTY_PTR_ARRAY(openremoteslide_get_property_names(osr));
  CHECK_RET(openremoteslide_get_property_value(osr, OPENREMOTESLIDE_PROPERTY_NAME_VENDOR),
            NULL);
  CHECK_EMPTY_PTR_ARRAY(openremoteslide_get_associated_image_names(osr));
  CHECK_W_H(openremoteslide_get_associated_image_dimensions(osr, "label", &w, &h),
            -1, -1);
  CHECK_W_H(openremoteslide_get_associated_image_dimensions(osr, "macro", &w, &h),
            -1, -1);

  openremoteslide_read_region(osr, NULL, 0, 0, 0, 10, 10);
  openremoteslide_read_associated_image(osr, "label", NULL);
  openremoteslide_read_associated_image(osr, "macro", NULL);
}

static void check_props(openremoteslide_t *osr) {
  for (gchar **check = prop_checks; !have_error && check && *check; check++) {
    ...
    const char *value = openremoteslide_get_property_value(osr, key);
    check_error(osr);
...
static void check_regions(openremoteslide_t *osr) {
...
int main(int argc, char **argv) {
  ...
  openremoteslide_t *osr = openremoteslide_open(filename);
  ...
  if (osr != NULL) {
    ...
    // openremoteslide_open returned NULL but logged nothing
    have_error = TRUE;
  }
  ...
  // Do timing run.  The earlier openremoteslide_open() doesn't count because
  ...
  if (time_check && !have_error) {
    ...
    for (int i = 0; i < TIME_ITERATIONS; i++) {
      ...
      osr = openremoteslide_open(filename);
      g_timer_stop(timer);
      ...
      if (osr != NULL) {
        ...
        fail("openremoteslide_open() returned NULL during timing loop");
      }
      if (have_error) {
        break;
      }
