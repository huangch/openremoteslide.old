...
static void print_downsamples(openremoteslide_t *osr) {
  for (int32_t level = 0; level < openremoteslide_get_level_count(osr); level++) {
    printf("level %d: downsample: %g\n",
	   level,
	   openremoteslide_get_level_downsample(osr, level));
  }
}

static void test_next_biggest(openremoteslide_t *osr, double downsample) {
  int32_t level = openremoteslide_get_best_level_for_downsample(osr, downsample);
  printf("level for downsample %g: %d (%g)\n",
	 downsample, level, openremoteslide_get_level_downsample(osr, level));
}

/*
static void test_tile_walk(openremoteslide_t *osr,
			   int64_t tile_size) {
  ...
  for (int64_t y = 0; y < h; y += tile_size) {
    for (int64_t x = 0; x < w; x += tile_size) {
      ...
      openremoteslide_read_region(osr, buf, x, y, 0, tile_size, tile_size);
...
static void test_image_fetch(openremoteslide_t *osr,
			     const char *name,
			     int64_t x, int64_t y,
			     int64_t w, int64_t h,
			     bool skip_write) {
  ...
  const char *bgcolor = openremoteslide_get_property_value(osr, OPENREMOTESLIDE_PROPERTY_NAME_BACKGROUND_COLOR);
  if (bgcolor) {
  ...
  for (int32_t level = 0; level < openremoteslide_get_level_count(osr); level++) {
    ...
    openremoteslide_read_region(osr, buf, x, y, level, w, h);
...
/*
static void test_horizontal_walk(openremoteslide_t *osr,
				 int64_t start_x,
				 int64_t y,
				 int32_t level,
  ...
  for (int64_t x = start_x; x < d; x += stride) {
    openremoteslide_read_region(osr, buf, x, y, level, patch_w, patch_h);
    printf("%"PRId64"\r", x);
    fflush(stdout);
  }
...
static void test_vertical_walk(openremoteslide_t *osr,
			       int64_t x,
			       int64_t start_y,
			       int32_t level,
			       int64_t patch_w, int64_t patch_h,
...
static void test_pdf(openremoteslide_t *osr, const char *filename) {
  printf("test_pdf: %s\n", filename);
  cairo_surface_t *pdf = cairo_pdf_surface_create(filename, 0, 0);
  cairo_t *cr = cairo_create(pdf);
  cairo_rotate(cr, M_PI_4);

  for (int i = 0; i < openremoteslide_get_level_count(osr); i++) {
    int64_t orig_w, orig_h;
    openremoteslide_get_level_dimensions(osr, i, &orig_w, &orig_h);
    ...
    openremoteslide_cairo_read_region(osr, cr, (orig_w - w) / 2, (orig_h - h) / 2, i, w, h);
    printf(".");
    fflush(stdout);
...
int main(int argc, char **argv) {
  ...
  printf("version: %s\n", openremoteslide_get_version());

  printf("openremoteslide_detect_vendor returns %s\n", openremoteslide_detect_vendor(argv[1]));
  openremoteslide_t *osr = openremoteslide_open(argv[1]);

  int64_t w, h;

  if (osr == NULL || openremoteslide_get_error(osr) != NULL) {
    printf("oh no\n");
    exit(1);
  }
  ...
  osr = openremoteslide_open(argv[1]);
  ...
  int32_t levels = openremoteslide_get_level_count(osr);
  printf("num levels: %d\n", levels);

  for (int32_t i = -1; i < levels + 1; i++) {
    int64_t ww, hh;
    openremoteslide_get_level_dimensions(osr, i, &ww, &hh);
  ...
  // test NULL dest
  openremoteslide_read_region(osr, NULL, 0, 0, 0, 1000, 1000);

  // test empty dest
  uint32_t* item = 0;
  openremoteslide_read_region(osr, item, 0, 0, 0, 0, 0);
  ...
  openremoteslide_cairo_read_region(osr, cr, 0, 0, 0, 1000, 1000);
  cairo_destroy(cr);
  */
  ...
  const char * const *associated_image_names = openremoteslide_get_associated_image_names(osr);
  while (*associated_image_names) {
    int64_t w;
    int64_t h;
    const char *name = *associated_image_names;
    openremoteslide_get_associated_image_dimensions(osr, name, &w, &h);

    printf("associated image: %s -> (%"PRId64"x%"PRId64")\n", name, w, h);

    uint32_t *buf = g_new(uint32_t, w * h);
    openremoteslide_read_associated_image(osr, name, buf);
    g_free(buf);
