...
static void check_cloexec_leaks(const char *slide, void *prog,
  ...
  while (g_timer_elapsed(timer, NULL) < 2) {
    openremoteslide_t *osr = openremoteslide_open(slide);
    openremoteslide_read_region(osr, buf, x, y, 0, 512, 512);
...
int main(int argc, char **argv) {
  ...
  if (!openremoteslide_detect_vendor(path)) {
  ...
  osr = openremoteslide_open(path);
  if (!osr || openremoteslide_get_error(osr)) {
    fail("Reopen failed");
  }
  ...
  for (int32_t i = -1; i < levels + 1; i++) {
    ...
    openremoteslide_get_level_downsample(osr, i);
  }

  openremoteslide_get_best_level_for_downsample(osr, 0.8);
  openremoteslide_get_best_level_for_downsample(osr, 1.0);
  openremoteslide_get_best_level_for_downsample(osr, 1.5);
  openremoteslide_get_best_level_for_downsample(osr, 2.0);
  openremoteslide_get_best_level_for_downsample(osr, 3.0);
  openremoteslide_get_best_level_for_downsample(osr, 3.1);
  openremoteslide_get_best_level_for_downsample(osr, 10);
  openremoteslide_get_best_level_for_downsample(osr, 20);
  openremoteslide_get_best_level_for_downsample(osr, 25);
  openremoteslide_get_best_level_for_downsample(osr, 100);
  openremoteslide_get_best_level_for_downsample(osr, 1000);
  openremoteslide_get_best_level_for_downsample(osr, 10000);

  // NULL buffer
  openremoteslide_read_region(osr, NULL, 0, 0, 0, 1000, 1000);

  // empty region
  openremoteslide_read_region(osr, NULL, 0, 0, 0, 0, 0);

  // read properties
  const char * const *property_names = openremoteslide_get_property_names(osr);
  while (*property_names) {
    const char *name = *property_names;
    openremoteslide_get_property_value(osr, name);
    property_names++;
  }

  // read associated images
  const char * const *associated_image_names =
    openremoteslide_get_associated_image_names(osr);
