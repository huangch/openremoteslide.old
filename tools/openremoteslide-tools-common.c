...
static GOptionContext *make_option_context(const struct openremoteslide_tools_usage_info *info) {
...
void _openremoteslide_tools_parse_commandline(const struct openremoteslide_tools_usage_info *info,
                                        int *argc,
                                        char ***argv) {
  GError *err = NULL;
  ...
  if (err) {
    ...
    g_error_free(err);
    _openremoteslide_tools_usage(info);

  } else if (show_version) {
    fprintf(stderr, version_format, g_get_prgname(), openremoteslide_get_version());
    exit(0);
  }
...
void _openremoteslide_tools_usage(const struct openremoteslide_tools_usage_info *info) {
  GOptionContext *octx = make_option_context(info);
