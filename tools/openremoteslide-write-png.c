...
static const char OPENREMOTESLIDE[] = "OpenSlide <http://openremoteslide.org/>";
...
static void write_png(openremoteslide_t *osr, FILE *f,
  ...
  text_ptr[0].key = key;
  char *text = strdup(OPENREMOTESLIDE);
  ...
  // background
  const char *bgcolor = openremoteslide_get_property_value(osr,
						     OPENREMOTESLIDE_PROPERTY_NAME_BACKGROUND_COLOR);
  if (bgcolor) {
    int r, g, b;
  ...
  int32_t yy = y / ds;
  while (lines_to_draw) {
    openremoteslide_read_region(osr, dest,
			  x, yy * ds, level, w, 1);
...
int main (int argc, char **argv) {
  ...
  // validate args
  ENSURE_NONNEG(level);
  if (level > openremoteslide_get_level_count(osr) - 1) {
    fail("level %d out of range (level count %d)",
	 level, openremoteslide_get_level_count(osr));
  }
  ENSURE_POS(width);
  ENSURE_POS(height);
  if (width > INT32_MAX) {
