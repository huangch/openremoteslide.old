...
struct openremoteslide_jpeg_error_mgr {
  struct jpeg_error_mgr base;
  jmp_buf *env;
  GError *err;
};

struct _openremoteslide_jpeg_decompress {
  struct jpeg_decompress_struct cinfo;
  struct openremoteslide_jpeg_error_mgr jerr;
...
static void my_error_exit(j_common_ptr cinfo) {
  struct openremoteslide_jpeg_error_mgr *jerr =
    (struct openremoteslide_jpeg_error_mgr *) cinfo->err;
...
static void my_output_message(j_common_ptr cinfo) {
  ...
  g_set_error(&jerr->err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_FAILED,
              "%s", buffer);
}
...
static struct jpeg_error_mgr *error_handler_init(struct openremoteslide_jpeg_error_mgr *jerr,
                                                 jmp_buf *env) {
...
static void *detect_jcs_alpha_extensions(void *arg G_GNUC_UNUSED) {
  ...
  struct openremoteslide_jpeg_error_mgr *jerr =
    g_slice_new0(struct openremoteslide_jpeg_error_mgr);
  ...
  if (!setjmp(env)) {
    ...
    _openremoteslide_performance_warn("Optimized libjpeg color space not available");
  }

  jpeg_destroy_compress(cinfo);
  g_slice_free(struct jpeg_compress_struct, cinfo);
  g_slice_free(struct openremoteslide_jpeg_error_mgr, jerr);
  //g_debug("have JCS_ALPHA_EXTENSIONS: %d", alpha_extensions);
  return GINT_TO_POINTER(alpha_extensions);
}

// the caller must assign the struct _openremoteslide_jpeg_decompress * before
...
struct _openremoteslide_jpeg_decompress *_openremoteslide_jpeg_decompress_create(struct jpeg_decompress_struct **out_cinfo) {
  struct _openremoteslide_jpeg_decompress *dc = g_slice_new0(struct _openremoteslide_jpeg_decompress);
  *out_cinfo = &dc->cinfo;
  return dc;
}
...
void _openremoteslide_jpeg_decompress_destroy(struct _openremoteslide_jpeg_decompress *dc) {
  ...
  g_slice_free(struct _openremoteslide_jpeg_decompress, dc);
}
...
static bool jpeg_get_dimensions(URLIO_FILE *f,  // or:
  ...
  if (setjmp(env) == 0) {
    ...
    if (f) {
      _openremoteslide_jpeg_stdio_src(cinfo, f);
    } else {
...
static bool jpeg_decode(URLIO_FILE *f,  // or:
  ...
  if (setjmp(env) == 0) {
    ...
    // decompress
    if (!_openremoteslide_jpeg_decompress_run(dc, dest, grayscale, w, h, err)) {
      goto DONE;
    }
    result = true;
  } else {
...
static bool get_associated_image_data(struct _openremoteslide_associated_image *_img,
  ...
  return _openremoteslide_jpeg_read(img->filename, img->offset, dest,
...
static const struct _openremoteslide_associated_image_ops jpeg_associated_ops = {
...
bool _openremoteslide_jpeg_add_associated_image(openremoteslide_t *osr,
					  ...
					  GError **err) {
  int32_t w, h;
  if (!_openremoteslide_jpeg_read_dimensions(filename, offset, &w, &h, err)) {
