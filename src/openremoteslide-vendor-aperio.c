...
static bool render_missing_tile(struct level *l,
  ...
  if (l->prev) {
    ...
    success = _openremoteslide_grid_paint_region(l->prev->grid, cr, tiff,
                                           (tile_col * tw - 1) / relative_ds,
                                           (tile_row * th - 1) / relative_ds,
                                           (struct _openremoteslide_level *) l->prev,
                                           ceil((tw + 2) / relative_ds),
                                           ceil((th + 2) / relative_ds),
                                           err);
    if (success) {
      success = _openremoteslide_check_cairo_status(cr, err);
    }
    cairo_destroy(cr);
  }
...
static bool decode_tile(struct level *l,
  ...
  // select color space
  enum _openremoteslide_jp2k_colorspace space;
  switch (l->compression) {
  case APERIO_COMPRESSION_JP2K_YCBCR:
    space = OPENREMOTESLIDE_JP2K_YCBCR;
    break;
  case APERIO_COMPRESSION_JP2K_RGB:
    space = OPENREMOTESLIDE_JP2K_RGB;
    break;
  default:
    // not for us? fallback
    return _openremoteslide_tiff_read_tile(tiffl, tiff, dest,
                                     tile_col, tile_row,
                                     err);
  }
  ...
  // decompress
  bool success = _openremoteslide_jp2k_decode_buffer(dest,
...
static const struct _openremoteslide_ops aperio_ops = {
...
static bool aperio_detect(const char *filename G_GNUC_UNUSED,
  ...
  const char *tagval = _openremoteslide_tifflike_get_buffer(tl, 0,
...
static void add_properties(openremoteslide_t *osr, char **props) {
  if (*props == NULL) {
    return;
  }
  ...
  _openremoteslide_duplicate_int_prop(osr, "aperio.AppMag",
                                OPENREMOTESLIDE_PROPERTY_NAME_OBJECTIVE_POWER);
  _openremoteslide_duplicate_double_prop(osr, "aperio.MPP",
...
static bool add_associated_image(openremoteslide_t *osr,
  ...
  bool result = _openremoteslide_tiff_add_associated_image(osr, name, tc,
...
static void propagate_missing_tile(void *key, void *value G_GNUC_UNUSED,
  ...
  struct _openremoteslide_tiff_level *tiffl = &l->tiffl;
  struct _openremoteslide_tiff_level *next_tiffl = &next_l->tiffl;
...
// (see openremoteslide-decode-jp2k.c)
...
static bool aperio_open(openremoteslide_t *osr,
                        const char *filename,
  ...
  do {
    ...
    if (TIFFIsTiled(tiff)) {
      ...
      if (!_openremoteslide_tiff_level_init(tiff,
                                      dir,
                                      (struct _openremoteslide_level *) l,
                                      tiffl,
                                      err)) {
        goto FAIL;
      }
...
const struct _openremoteslide_format _openremoteslide_format_aperio = {
  .name = "aperio",
  .vendor = "aperio",
