...
static void unpack_argb(enum _openremoteslide_jp2k_colorspace space,
  ...
  if (space == OPENREMOTESLIDE_JP2K_YCBCR &&
    ...
    for (int32_t y = 0; y < h; y++) {
      ...
      for (x = 0; x < w - 1; x += 2) {
        ...
        int16_t R_chroma = _openremoteslide_R_Cr[c2];
        int16_t G_chroma = (_openremoteslide_G_Cb[c1] + _openremoteslide_G_Cr[c2]) >> 16;
        int16_t B_chroma = _openremoteslide_B_Cb[c1];
  ...
  } else if (space == OPENREMOTESLIDE_JP2K_YCBCR) {
    // Slow fallback
    static gint warned_slowpath_ycbcr;
    _openremoteslide_performance_warn_once(&warned_slowpath_ycbcr,
  ...
  } else if (space == OPENREMOTESLIDE_JP2K_RGB &&
    ...
    _openremoteslide_performance_warn_once(&warned_slowpath_rgb,
...
bool _openremoteslide_jp2k_decode_buffer(uint32_t *dest,
                                   int32_t w, int32_t h,
...
bool _openremoteslide_jp2k_decode_buffer(uint32_t *dest,
  ...
  if (tmp_err) {
    ...
    // http://lists.andrew.cmu.edu/pipermail/openremoteslide-users/2014-March/000751.html
    //
