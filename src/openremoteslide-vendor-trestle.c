...
static const struct _openremoteslide_ops trestle_ops = {
...
static bool trestle_detect(const char *filename G_GNUC_UNUSED,
  ...
  if (!_openremoteslide_tifflike_get_buffer(tl, 0, TIFFTAG_IMAGEDESCRIPTION, err)) {
    return false;
  }

  // ensure all levels are tiled
  int64_t dirs = _openremoteslide_tifflike_get_directory_count(tl);
  for (int64_t i = 0; i < dirs; i++) {
    if (!_openremoteslide_tifflike_is_tiled(tl, i)) {
      g_set_error(err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_FAILED,
...
static void add_properties(openremoteslide_t *osr, char **tags) {
  ...
  _openremoteslide_duplicate_int_prop(osr, "trestle.Objective Power",
                                OPENREMOTESLIDE_PROPERTY_NAME_OBJECTIVE_POWER);
}

static void parse_trestle_image_description(openremoteslide_t *osr,
...
static void add_associated_jpeg(openremoteslide_t *osr, TIFF *tiff,
                                const char *extension,
                                const char *name) {
  char *path = get_associated_path(tiff, extension);
  _openremoteslide_jpeg_add_associated_image(osr, name, path, 0, NULL);
  g_free(path);
}

static bool trestle_open(openremoteslide_t *osr, const char *filename,
                         struct _openremoteslide_tifflike *tl,
  ...
  for (int32_t i = 0; i < level_count; i++) {
    ...
    if (!_openremoteslide_tiff_level_init(tiff, i,
  ...
  _openremoteslide_duplicate_double_prop(osr, "tiff.XResolution",
                                   OPENREMOTESLIDE_PROPERTY_NAME_MPP_X);
  _openremoteslide_duplicate_double_prop(osr, "tiff.YResolution",
...
const struct _openremoteslide_format _openremoteslide_format_trestle = {
  .name = "trestle",
  .vendor = "trestle",
