...
#define PARSE_DOUBLE_ATTRIBUTE_OR_FAIL(NODE, NAME, OUT)			\
  do {									\
    GError *tmp_err = NULL;						\
    OUT = _openremoteslide_xml_parse_double_attr(NODE, NAME, &tmp_err);	\
    if (tmp_err)  {							\
      g_propagate_error(err, tmp_err);					\
      goto FAIL;							\
    }									\
...
static bool read_subtile(openremoteslide_t *osr,
                         cairo_t *cr,
  ...
  struct _openremoteslide_tiff_level *tiffl = &l->tiffl;
  TIFF *tiff = arg;
  bool success = true;
  ...
  if (!tiledata) {
    ...
    if (!_openremoteslide_tiff_read_tile(tiffl, tiff,
    ...
    // put it in the cache
    _openremoteslide_cache_put(osr->cache, level, tile_col, tile_row,
                         tiledata, tw * th * 4,
                         &cache_entry);
  }
  ...
  if (l->subtiles_per_tile > 1) {
    ...
    cairo_rectangle(cr2, 0, 0,
                    ...
                    ceil(subtile_h));
    cairo_fill(cr2);
    success = _openremoteslide_check_cairo_status(cr2, err);
    cairo_destroy(cr2);
  }
...
// the tilemap grid
static bool read_subtile_tilemap(openremoteslide_t *osr,
                                 cairo_t *cr,
                                 struct _openremoteslide_level *level,
                                 int64_t subtile_col, int64_t subtile_row,
...
static bool paint_region(openremoteslide_t *osr, cairo_t *cr,
  ...
  bool success = _openremoteslide_grid_paint_region(l->grid, cr, tiff,
                                              x / l->base.downsample,
                                              y / l->base.downsample,
                                              level, w, h,
                                              err);
...
static const struct _openremoteslide_ops ventana_ops = {
...
static bool ventana_detect(const char *filename G_GNUC_UNUSED,
  ...
  // read XMLPacket
  const char *xml = _openremoteslide_tifflike_get_buffer(tl, 0, TIFFTAG_XMLPACKET,
                                                   err);
  if (!xml) {
    return false;
  }
...
static bool parse_initial_xml(openremoteslide_t *osr, const char *xml,
                              GError **err) {
  // parse
  xmlDoc *doc = _openremoteslide_xml_parse(xml, err);
  if (!doc) {
  ...
  _openremoteslide_duplicate_int_prop(osr, "ventana.Magnification",
                                OPENREMOTESLIDE_PROPERTY_NAME_OBJECTIVE_POWER);
  _openremoteslide_duplicate_double_prop(osr, "ventana.ScanRes",
...
static struct bif *parse_level0_xml(const char *xml,
  ...
  // query AOI metadata
  info_result =
    _openremoteslide_xml_xpath_eval(ctx, "/EncodeInfo/SlideStitchInfo/ImageInfo");
  origin_result =
    _openremoteslide_xml_xpath_eval(ctx, "/EncodeInfo/AoiOrigin/*");
  ...
  for (int i = 0; i < info_result->nodesetval->nodeNr; i++) {
    ...
    // walk tiles
    ctx->node = info;
    result = _openremoteslide_xml_xpath_eval(ctx, "TileJointInfo");
    if (!result) {
      g_set_error(err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_FAILED,
...
static bool parse_level_info(const char *desc,
  ...
  *level = g_ascii_strtoll(level_str, &endptr, 10);
  ...
  // parse magnification
  *magnification = _openremoteslide_parse_double(magnification_str);
...
static struct _openremoteslide_grid *create_bif_grid(openremoteslide_t *osr,
                                               struct bif *bif,
                                               double downsample,
  ...
  struct _openremoteslide_grid *grid =
    _openremoteslide_grid_create_tilemap(osr,
  ...
  for (int32_t i = 0; i < bif->num_areas; i++) {
    ...
    for (int64_t row = area->start_row;
      ...
      for (int64_t col = area->start_col;
        ...
        _openremoteslide_grid_tilemap_add_tile(grid,
                                         col, row,
                                         offset_x, offset_y,
                                         subtile_w, subtile_h,
                                         NULL);
...
static void set_region_props(openremoteslide_t *osr, struct bif *bif,
                             struct level *level0) {
  for (int32_t i = 0; i < bif->num_areas; i++) {
    struct area *area = bif->areas[i];
    g_hash_table_insert(osr->properties,
                        g_strdup_printf(_OPENREMOTESLIDE_PROPERTY_NAME_TEMPLATE_REGION_X, i),
                        g_strdup_printf("%"PRId64, (int64_t) (bif->tile_advance_x * area->start_col)));
    g_hash_table_insert(osr->properties,
                        g_strdup_printf(_OPENREMOTESLIDE_PROPERTY_NAME_TEMPLATE_REGION_Y, i),
                        g_strdup_printf("%"PRId64, (int64_t) (bif->tile_advance_y * area->start_row)));
    g_hash_table_insert(osr->properties,
                        g_strdup_printf(_OPENREMOTESLIDE_PROPERTY_NAME_TEMPLATE_REGION_WIDTH, i),
                        g_strdup_printf("%"PRId64, (int64_t) ceil(bif->tile_advance_x * (area->tiles_across - 1) + level0->tiffl.tile_w)));
    g_hash_table_insert(osr->properties,
                        g_strdup_printf(_OPENREMOTESLIDE_PROPERTY_NAME_TEMPLATE_REGION_HEIGHT, i),
                        g_strdup_printf("%"PRId64, (int64_t) ceil(bif->tile_advance_y * (area->tiles_down - 1) + level0->tiffl.tile_h)));
  }
}

static bool ventana_open(openremoteslide_t *osr, const char *filename,
                         struct _openremoteslide_tifflike *tl,
  ...
  do {
    ...
    if (strstr(image_desc, LEVEL_DESCRIPTION_TOKEN)) {
      ...
      if (level == 0) {
        // get XML
        xml = _openremoteslide_tifflike_get_buffer(tl, dir, TIFFTAG_XMLPACKET,
                                             &tmp_err);
        if (xml) {
          // get tile size
          struct _openremoteslide_tiff_level tiffl;
          if (!_openremoteslide_tiff_level_init(tiff, dir, NULL, &tiffl, err)) {
            goto FAIL;
          }
          // parse
        ...
        } else if (g_error_matches(tmp_err, OPENREMOTESLIDE_ERROR,
      ...
      struct _openremoteslide_tiff_level *tiffl = &l->tiffl;
      if (!_openremoteslide_tiff_level_init(tiff, dir,
                                      &l->base, tiffl,
                                      err)) {
        g_slice_free(struct level, l);
        goto FAIL;
      ...
      if (bif) {
        ...
        double x, y, w, h;
        _openremoteslide_grid_get_bounds(l->grid, &x, &y, &w, &h);
        l->base.w = ceil(x + w);
        l->base.h = ceil(y + h);
        // clear tile size hints set by _openremoteslide_tiff_level_init()
        l->base.tile_w = 0;
        l->base.tile_h = 0;
      } else {
      ...
      // _openremoteslide_tiff_read_tile() uses the directory's tile size
      ...
      // macro image
      if (!_openremoteslide_tiff_add_associated_image(osr, "macro", tc, dir,
                                                err)) {
	goto FAIL;
      }
      ...
      // thumbnail image
      if (!_openremoteslide_tiff_add_associated_image(osr, "thumbnail", tc, dir,
                                                err)) {
	goto FAIL;
      }
    }
...
const struct _openremoteslide_format _openremoteslide_format_ventana = {
  .name = "ventana",
  .vendor = "ventana",
