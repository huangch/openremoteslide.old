...
static bool read_tile(openremoteslide_t *osr,
  ...
  if (!tiledata) {
    ...
    bool is_missing;
    if (!_openremoteslide_tiff_check_missing_tile(tiffl, tiff,
                                            tile_col, tile_row,
                                            &is_missing, err)) {
      return false;
    }
    ...
    if (is_missing) {
      ...
      // clip, if necessary
      if (!_openremoteslide_clip_tile(tiledata,
                                tw, th,
...
static const struct _openremoteslide_ops philips_ops = {
...
static bool philips_detect(const char *filename G_GNUC_UNUSED,
  ...
  const char *software = _openremoteslide_tifflike_get_buffer(tl, 0,
                                                        TIFFTAG_SOFTWARE,
                                                        err);
  if (!software) {
    return false;
  ...
  const char *image_desc =
    _openremoteslide_tifflike_get_buffer(tl, 0, TIFFTAG_IMAGEDESCRIPTION, err);
  if (!image_desc) {
    return false;
  }
...
static xmlDoc *parse_xml(TIFF *tiff, GError **err) {
  if (!_openremoteslide_tiff_set_dir(tiff, 0, err)) {
    return NULL;
  }
  ...
  return _openremoteslide_xml_parse(image_desc, err);
}
...
static bool get_compressed_xml_associated_image_data(xmlDoc *doc,
                                                     ...
                                                     gsize *out_len,
                                                     GError **err) {
  xmlXPathContext *ctx = _openremoteslide_xml_xpath_create(doc);
  char *b64_data = _openremoteslide_xml_xpath_get_string(ctx, xpath);
  if (b64_data) {
...
static bool get_xml_associated_image_data(struct _openremoteslide_associated_image *_img,
                                          uint32_t *dest,
                                          GError **err) {
  ...
  success = _openremoteslide_jpeg_decode_buffer(data, len, dest,
...
static void destroy_xml_associated_image(struct _openremoteslide_associated_image *_img) {
  struct xml_associated_image *img = (struct xml_associated_image *) _img;

  g_slice_free(struct xml_associated_image, img);
}

static const struct _openremoteslide_associated_image_ops philips_xml_associated_ops = {
...
static bool maybe_add_xml_associated_image(openremoteslide_t *osr,
                                           struct _openremoteslide_tiffcache *tc,
                                           xmlDoc *doc,
                                           const char *name,
                                           const char *xpath,
  ...
  int32_t w, h;
  bool success =
    _openremoteslide_jpeg_decode_buffer_dimensions(data, len, &w, &h, err);
  g_free(data);
  if (!success) {
...
static void add_properties_from_array(openremoteslide_t *osr,
                                      xmlXPathContext *ctx,
                                      const char *prefix,
                                      xmlNode *node) {
  xmlChar *name = xmlGetProp(node, BAD_CAST XML_NAME_ATTR);
  ctx->node = node;
  xmlXPathObject *result = _openremoteslide_xml_xpath_eval(ctx, "Array/DataObject");
...
static void add_properties(openremoteslide_t *osr,
                           ...
                           const char *prefix,
                           const char *xpath) {
  xmlXPathObject *result = _openremoteslide_xml_xpath_eval(ctx, xpath);
  for (int i = 0; result && i < result->nodesetval->nodeNr; i++) {
...
static bool parse_pixel_spacing(const char *spacing,
  ...
  if (g_strv_length(spacings) == 2) {
    ...
    *w = _openremoteslide_parse_double(spacings[1]);
    *h = _openremoteslide_parse_double(spacings[0]);
...
static void add_mpp_properties(openremoteslide_t *osr) {
  ...
  if (spacing) {
    ...
    if (parse_pixel_spacing(spacing, &w, &h, NULL)) {
      g_hash_table_insert(osr->properties,
                          g_strdup(OPENREMOTESLIDE_PROPERTY_NAME_MPP_X),
                          _openremoteslide_format_double(1e3 * w));
      g_hash_table_insert(osr->properties,
                          g_strdup(OPENREMOTESLIDE_PROPERTY_NAME_MPP_Y),
                          _openremoteslide_format_double(1e3 * h));
    }
  }
}
...
static bool fix_level_dimensions(struct level **levels,
  ...
  xmlXPathObject *result =
    _openremoteslide_xml_xpath_eval(ctx,
                              "/DataObject"
...
static bool verify_main_image_count(xmlDoc *doc, GError **err) {
  xmlXPathContext *ctx = _openremoteslide_xml_xpath_create(doc);
  xmlXPathObject *result = _openremoteslide_xml_xpath_eval(ctx, MAIN_IMAGE_XPATH);
...
static bool philips_open(openremoteslide_t *osr,
                         const char *filename,
                         struct _openremoteslide_tifflike *tl,
  ...
  do {
    ...
    if (TIFFIsTiled(tiff)) {
      ...
      if (!_openremoteslide_tiff_level_init(tiff, dir,
                                      (struct _openremoteslide_level *) l, tiffl,
                                      err)) {
        g_slice_free(struct level, l);
        goto FAIL;
      }
      ...
      if (!_openremoteslide_tiff_add_associated_image(osr, "label", tc, dir, err)) {
        goto FAIL;
      }
      ...
      if (!_openremoteslide_tiff_add_associated_image(osr, "macro", tc, dir, err)) {
        goto FAIL;
      }
    }
...
const struct _openremoteslide_format _openremoteslide_format_philips = {
  .name = "philips",
  .vendor = "philips",
