...
#define PREPARE_OR_FAIL(DEST, DB, SQL) do {				\
    DEST = _openremoteslide_sqlite_prepare(DB, SQL, err);			\
    if (!DEST) {							\
      goto FAIL;							\
    }									\
  } while (0)
...
#define BIND_TEXT_OR_FAIL(STMT, INDEX, STR) do {			\
    if (sqlite3_bind_text(STMT, INDEX, STR, -1, SQLITE_TRANSIENT)) {	\
      _openremoteslide_sqlite_propagate_stmt_error(STMT, err);		\
      goto FAIL;							\
    }									\
  } while (0)

#define STEP_OR_FAIL(STMT) do {						\
    if (!_openremoteslide_sqlite_step(STMT, err)) {				\
      goto FAIL;							\
    }									\
  } while (0)
...
static bool sakura_detect(const char *filename,
  ...
  // open database
  sqlite3 *db = _openremoteslide_sqlite_open(filename, err);
  if (!db) {
    return false;
  }
...
static bool read_channel(uint8_t *channeldata,
  ...
  // decompress
  return _openremoteslide_jpeg_decode_buffer_gray(buf, buflen, channeldata,
...
static bool read_tile(openremoteslide_t *osr,
  ...
  if (!tiledata) {
    ...
    if (!read_image(tiledata, tile_col, tile_row, l->base.downsample,
      ...
      if (g_error_matches(tmp_err, OPENREMOTESLIDE_ERROR,
...
static bool paint_region(openremoteslide_t *osr, cairo_t *cr,
  ...
  sqlite3 *db = _openremoteslide_sqlite_open(data->filename, err);
  if (!db) {
    return false;
  }
  PREPARE_OR_FAIL(stmt, db, data->data_sql);

  success = _openremoteslide_grid_paint_region(l->grid, cr, stmt,
                                         x / l->base.downsample,
                                         y / l->base.downsample,
                                         level, w, h,
                                         err);
...
static const struct _openremoteslide_ops sakura_ops = {
...
static bool get_associated_image_data(struct _openremoteslide_associated_image *_img,
  ...
  // open DB handle
  sqlite3 *db = _openremoteslide_sqlite_open(img->filename, err);
  if (!db) {
    return false;
  }
  ...
  // decode it
  success = _openremoteslide_jpeg_decode_buffer(buf, buflen, dest,
...
static const struct _openremoteslide_associated_image_ops sakura_associated_ops = {
...
static bool add_associated_image(openremoteslide_t *osr,
  ...
  int32_t w, h;
  if (!_openremoteslide_jpeg_decode_buffer_dimensions(buf, buflen, &w, &h, err)) {
    goto FAIL;
  }
...
static void add_properties(openremoteslide_t *osr,
  ...
  // execute it
  sqlite3_stmt *stmt = _openremoteslide_sqlite_prepare(db, sql, NULL);
  ...
  stmt = _openremoteslide_sqlite_prepare(db, "SELECT ResolutionMmPerPix FROM "
  ...
  if (stmt && sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    g_hash_table_insert(osr->properties,
                        ...
                        _openremoteslide_format_double(mmpp * 1000));
    g_hash_table_insert(osr->properties,
                        g_strdup(OPENREMOTESLIDE_PROPERTY_NAME_MPP_Y),
                        _openremoteslide_format_double(mmpp * 1000));
  }
  sqlite3_finalize(stmt);
  _openremoteslide_duplicate_double_prop(osr, "sakura.NominalLensMagnification",
                                   OPENREMOTESLIDE_PROPERTY_NAME_OBJECTIVE_POWER);

  // add version property
  sql = g_strdup_printf("SELECT data FROM %s WHERE id = '++VersionBytes'",
                        unique_table_name);
  stmt = _openremoteslide_sqlite_prepare(db, sql, NULL);
  if (stmt && sqlite3_step(stmt) == SQLITE_ROW) {
...
static bool hash_columns(struct _openremoteslide_hash *quickhash1,
                         sqlite3 *db,
                         const char *sql,
                         GError **err) {
  bool success = false;
  ...
  while ((ret = sqlite3_step(stmt)) == SQLITE_ROW) {
    for (int i = 0; i < sqlite3_column_count(stmt); i++) {
      ...
      _openremoteslide_hash_data(quickhash1, data, datalen);
      _openremoteslide_hash_data(quickhash1, "", 1);
    }
  }
  if (ret != SQLITE_DONE) {
    _openremoteslide_sqlite_propagate_error(db, err);
    goto FAIL;
  }
...
static bool hash_tiles(struct _openremoteslide_hash *quickhash1,
                       sqlite3 *db,
                       const char *unique_table_name,
                       GQueue *tileids,
                       GError **err) {
...
static void compute_quickhash1(struct _openremoteslide_hash *quickhash1,
                               sqlite3 *db,
                               const char *unique_table_name,
                               GQueue *tileids) {
...
FAIL:
  _openremoteslide_hash_disable(quickhash1);
}
...
static bool sakura_open(openremoteslide_t *osr, const char *filename,
...
const struct _openremoteslide_format _openremoteslide_format_sakura = {
  .name = "sakura",
  .vendor = "sakura",
