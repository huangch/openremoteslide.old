 ...
 * NDPI quickhash comes from _openremoteslide_tifflike_init_properties_and_hash
 *
 */
...
#define TIFF_GET_UINT_OR_FAIL(TL, DIR, TAG, OUT) do {			\
    GError *tmp_err = NULL;						\
    OUT = _openremoteslide_tifflike_get_uint(TL, DIR, TAG, &tmp_err);		\
    if (tmp_err) {							\
...
struct jpeg_level {
  struct _openremoteslide_level base;
...
enum OpenSlideHamamatsuError {
  ...
  OPENREMOTESLIDE_HAMAMATSU_ERROR_NO_RESTART_MARKERS,
};
static GQuark _openremoteslide_hamamatsu_error_quark(void) {
  return g_quark_from_string("openremoteslide-hamamatsu-error-quark");
}
#define OPENREMOTESLIDE_HAMAMATSU_ERROR _openremoteslide_hamamatsu_error_quark()
...
static bool jpeg_random_access_src(j_decompress_ptr cinfo,
  ...
  if (urlio_fseek(infile, header_start_position, SEEK_SET)) {
    _openremoteslide_io_error(err, "Couldn't seek to header start");
    return false;
  }
  ...
  if (data_length) {
    ...
    if (urlio_fseek(infile, start_position, SEEK_SET)) {
      _openremoteslide_io_error(err, "Couldn't seek to data start");
      return false;
    }
  ...
  _openremoteslide_jpeg_mem_src(cinfo, buffer, buffer_size);
...
static void jpeg_level_free(gpointer data) {
  ...
  g_free(l->jpegs);
  _openremoteslide_grid_destroy(l->grid);
...
static bool find_bitstream_start(URLIO_FILE *f,
  ...
  while (true) {
    ...
    if (urlio_fseek(f, pos + sizeof(buf) + len, SEEK_SET)) {
      _openremoteslide_io_error(err, "Couldn't seek to next marker");
      return false;
    }
...
static bool _compute_mcu_start(struct jpeg *jpeg,
  ...
  for (first_good = target; jpeg->mcu_starts[first_good] == -1; first_good--) {
    ...
    if (offset != -1) {
      ...
      if (urlio_fseek(f, offset - 2, SEEK_SET)) {
        _openremoteslide_io_error(err, "Couldn't seek to recorded restart "
  ...
  if (urlio_fseek(f, jpeg->mcu_starts[first_good], SEEK_SET)) {
    _openremoteslide_io_error(err, "Couldn't seek to first good restart marker");
    return false;
  }
...
static bool compute_mcu_start(openremoteslide_t *osr,
			      struct jpeg *jpeg,
			      URLIO_FILE *f,
			      int64_t tileno,
			      int64_t *start_position,
...
static bool compute_mcu_start_volatile(openremoteslide_t *osr,
                                       struct jpeg *jpeg,
                                       URLIO_FILE *f,
                                       int64_t tileno,
...
static bool read_from_jpeg(openremoteslide_t *osr,
                           struct jpeg *jpeg,
                           int32_t tileno,
                           int32_t scale_denom,
                           uint32_t *dest,
  ...
  // open file
  URLIO_FILE *f = _openremoteslide_fopen(jpeg->filename, "rb", err);
  if (f == NULL) {
    return false;
  }
...
static bool read_jpeg_tile(openremoteslide_t *osr,
                           cairo_t *cr,
  ...
  struct _openremoteslide_cache_entry *cache_entry;
  uint32_t *tiledata = _openremoteslide_cache_get(osr->cache,
  ...
  if (!tiledata) {
    ...
    _openremoteslide_cache_put(osr->cache,
			 level, tile_col, tile_row,
			 tiledata,
			 tw * th * 4,
			 &cache_entry);
  ...
  _openremoteslide_cache_entry_unref(cache_entry);

  return true;
}


static bool jpeg_paint_region(openremoteslide_t *osr, cairo_t *cr,
                              int64_t x, int64_t y,
                              struct _openremoteslide_level *level,
                              int32_t w, int32_t h,
                              GError **err) {
  ...
  // paint
  bool success = _openremoteslide_grid_paint_region(l->grid, cr, NULL,
                                              x / level->downsample,
                                              y / level->downsample,
                                              level, w, h,
                                              err);
...
static void jpeg_do_destroy(openremoteslide_t *osr) {
  struct hamamatsu_jpeg_ops_data *data = osr->data;
...
static const struct _openremoteslide_ops hamamatsu_jpeg_ops = {
...
static bool hamamatsu_vms_vmu_detect(const char *filename,
  ...
  if (!_openremoteslide_read_key_file(key_file, filename, KEY_FILE_MAX_SIZE,
                                G_KEY_FILE_NONE, err)) {
...
static bool verify_mcu_starts(int32_t num_jpegs, struct jpeg **jpegs,
  ...
  for (current_jpeg = 0; current_jpeg < num_jpegs; current_jpeg++) {
    ...
    CHK(jp->filename);
    f = _openremoteslide_fopen(jp->filename, "rb", NULL);
    CHK(f);
...
static gpointer restart_marker_thread_func(gpointer d) {
  ...
  while(current_jpeg < data->jpeg_count) {
	...
	current_file = _openremoteslide_fopen(jp->filename, "rb", &tmp_err);
	if (current_file == NULL) {
...
static bool validate_jpeg_header(URLIO_FILE *f, bool use_jpeg_dimensions,
  ...
  if (setjmp(env) == 0) {
    ...
    if (cinfo->restart_interval == 0) {
      g_set_error(err, OPENREMOTESLIDE_HAMAMATSU_ERROR,
                  OPENREMOTESLIDE_HAMAMATSU_ERROR_NO_RESTART_MARKERS,
                  "No restart markers");
      goto DONE;
    }
...
static int64_t *extract_one_optimisation(URLIO_FILE *opt_f,
                                         int32_t tiles_down,
                                         int32_t tiles_across) {
  ...
  for (int32_t row = 0; row < tiles_down; row++) {
    ...
    if (urlio_fread(u.buf, 40, 1, opt_f) != 1) {
...
static void add_mpp_property(openremoteslide_t *osr, GKeyFile *kf,
  ...
  if (nm > 0) {
    g_hash_table_insert(osr->properties,
                        g_strdup(property),
                        _openremoteslide_format_double(nm / (1000.0 * pixels)));
  }
}

static void add_properties(openremoteslide_t *osr,
                           GKeyFile *kf, const char *group,
                           struct _openremoteslide_level *level0) {
  ...
  // this allows openremoteslide.objective-power to have a fractional component
  // but it's better than rounding
  _openremoteslide_duplicate_double_prop(osr, "hamamatsu.SourceLens",
                                   OPENREMOTESLIDE_PROPERTY_NAME_OBJECTIVE_POWER);
  add_mpp_property(osr, kf, group, KEY_PHYSICAL_WIDTH, level0->w,
                   OPENREMOTESLIDE_PROPERTY_NAME_MPP_X);
  add_mpp_property(osr, kf, group, KEY_PHYSICAL_HEIGHT, level0->h,
                   OPENREMOTESLIDE_PROPERTY_NAME_MPP_Y);
}

// create scale_denom levels
static void create_scaled_jpeg_levels(openremoteslide_t *osr,
                                      int32_t *_level_count,
                                      struct jpeg_level ***_levels) {
  int32_t level_count = *_level_count;
  struct jpeg_level **levels = *_levels;

  GHashTable *expanded_levels = g_hash_table_new_full(_openremoteslide_int64_hash,
  ...
  for (int32_t i = 0; i < level_count; i++) {
    ...
    *key = l->base.w;
    ...
    for (int scale_denom = 2; scale_denom <= 8; scale_denom <<= 1) {
      ...
      // create grid
      sd_l->grid = _openremoteslide_grid_create_simple(osr,
                                                 sd_l->tiles_across,
                                                 sd_l->tiles_down,
                                                 sd_l->tile_width,
                                                 sd_l->tile_height,
  ...
  while (level_keys != NULL) {
    ...
    _openremoteslide_int64_free(level_keys->data);  // key
...
static bool init_jpeg_ops(openremoteslide_t *osr,
  ...
  osr->levels = (struct _openremoteslide_level **) levels;
  ...
  data->restart_marker_thread_throttle =
    !_openremoteslide_debug(OPENREMOTESLIDE_DEBUG_JPEG_MARKERS);
  ...
  // for debugging
  if (_openremoteslide_debug(OPENREMOTESLIDE_DEBUG_JPEG_MARKERS)) {
...
static struct jpeg_level *create_jpeg_level(openremoteslide_t *osr,
                                            struct jpeg **jpegs,
                                            int32_t jpeg_cols,
                                            int32_t jpeg_rows) {
  ...
  // create grid
  l->grid = _openremoteslide_grid_create_simple(osr,
...
static bool hamamatsu_vms_part2(openremoteslide_t *osr,
  ...
  for (int i = 0; i < num_jpegs; i++) {
    ...
    URLIO_FILE *f;
    if ((f = _openremoteslide_fopen(jp->filename, "rb", err)) == NULL) {
    ...
    if (comment) {
      g_hash_table_insert(osr->properties,
			  g_strdup(OPENREMOTESLIDE_PROPERTY_NAME_COMMENT),
			  comment);
    }

    if (urlio_fseek(f, 0, SEEK_END)) {
      _openremoteslide_io_error(err, "Can't seek to end of JPEG %d", i);
      urlio_fclose(f);
      goto FAIL;
    }
    jp->end_in_file = urlio_ftell(f);
    if (jp->end_in_file == -1) {
      _openremoteslide_io_error(err, "Can't read file size for JPEG %d", i);
      urlio_fclose(f);
      goto FAIL;
    }
    ...
    if (unreliable_mcu_starts) {
      ...
      if (i < num_jpegs - 1) {
        _openremoteslide_performance_warn("Bad optimisation file");
      }
    }
  }
...
static void ngr_destroy(openremoteslide_t *osr) {
...
static bool ngr_read_tile(openremoteslide_t *osr,
                          cairo_t *cr,
                          struct _openremoteslide_level *level,
  ...
  uint32_t *tiledata = _openremoteslide_cache_get(osr->cache, level, tile_x, tile_y,
                                            &cache_entry);

  if (!tiledata) {
    // read the tile data
    URLIO_FILE *f = _openremoteslide_fopen(l->filename, "rb", err);
    if (!f) {
      return false;
    }
    ...
    if (urlio_fseek(f, offset, SEEK_SET)) {
      _openremoteslide_io_error(err, "Couldn't seek to tile offset");
      urlio_fclose(f);
      return false;
    }
    ...
    // put it in the cache
    _openremoteslide_cache_put(osr->cache, level, tile_x, tile_y,
                         tiledata,
                         tilesize,
                         &cache_entry);
  }
...
static bool ngr_paint_region(openremoteslide_t *osr G_GNUC_UNUSED, cairo_t *cr,
                             int64_t x, int64_t y,
                             struct _openremoteslide_level *level,
                             int32_t w, int32_t h,
                             GError **err) {
  ...
  return _openremoteslide_grid_paint_region(l->grid, cr, NULL,
                                      x / level->downsample,
                                      y / level->downsample,
                                      level, w, h,
                                      err);
...
static const struct _openremoteslide_ops ngr_ops = {
...
static bool hamamatsu_vmu_part2(openremoteslide_t *osr,
  ...
  for (int i = 0; i < num_levels; i++) {
    ...
    URLIO_FILE *f;
    if ((f = _openremoteslide_fopen(l->filename, "rb", err)) == NULL) {
      goto FAIL;
    }
    ...
    if (urlio_fseek(f, 4, SEEK_SET)) {
      _openremoteslide_io_error(err, "Couldn't seek to NGR header");
      urlio_fclose(f);
      goto FAIL;
    }
    ...
    if (urlio_fseek(f, 24, SEEK_SET)) {
      _openremoteslide_io_error(err, "Couldn't seek within NGR header");
      urlio_fclose(f);
      goto FAIL;
    }
 ...
 FAIL:
  // destroy
  for (int i = 0; i < num_levels; i++) {
    _openremoteslide_grid_destroy(levels[i]->grid);
...
static bool hamamatsu_vms_vmu_open(openremoteslide_t *osr, const char *filename,
                                   struct _openremoteslide_tifflike *tl G_GNUC_UNUSED,
  ...
  if (!_openremoteslide_hash_file(quickhash1, filename, err)) {
    goto DONE;
  }
  ...
  if (tmp && *tmp) {
    ...
    if (!_openremoteslide_hash_file(quickhash1, map_filename, err)) {
      goto DONE;
    }
  } else {
    g_set_error(err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_FAILED,
  ...
  if (tmp && *tmp) {
    ...
    bool result = _openremoteslide_jpeg_add_associated_image(osr,
                                                       "macro",
  ...
  if (groupname == GROUP_VMS) {
    ...
    if (tmp) {
      ...
      optimisation_file = _openremoteslide_fopen(optimisation_filename, "rb", NULL);
    ...
    if (!optimisation_file) {
      _openremoteslide_performance_warn("Missing optimisation file");
    }
...
const struct _openremoteslide_format _openremoteslide_format_hamamatsu_vms_vmu = {
...
static bool hamamatsu_ndpi_detect(const char *filename G_GNUC_UNUSED,
  ...
  if (!_openremoteslide_tifflike_get_value_count(tl, 0, NDPI_FORMAT_FLAG)) {
    g_set_error(err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_FAILED,
...
static void ndpi_set_sint_prop(openremoteslide_t *osr,
                               struct _openremoteslide_tifflike *tl,
                               int64_t dir, int32_t tag,
                               const char *property_name) {
  GError *tmp_err = NULL;
  int64_t value = _openremoteslide_tifflike_get_sint(tl, dir, tag, &tmp_err);
  if (!tmp_err) {
    g_hash_table_insert(osr->properties,
...
static void ndpi_set_float_prop(openremoteslide_t *osr,
                                struct _openremoteslide_tifflike *tl,
                                int64_t dir, int32_t tag,
                                const char *property_name) {
  GError *tmp_err = NULL;
...
static void ndpi_set_resolution_prop(openremoteslide_t *osr,
                                     struct _openremoteslide_tifflike *tl,
                                     int64_t dir, int32_t tag,
                                     const char *property_name) {
  GError *tmp_err = NULL;
  uint64_t unit = _openremoteslide_tifflike_get_uint(tl, dir,
                                               TIFFTAG_RESOLUTIONUNIT,
                                               &tmp_err);
  if (g_error_matches(tmp_err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_NO_VALUE)) {
  ...
  double res = _openremoteslide_tifflike_get_float(tl, dir, tag, &tmp_err);
  if (!tmp_err && unit == RESUNIT_CENTIMETER) {
    g_hash_table_insert(osr->properties,
                        g_strdup(property_name),
                        _openremoteslide_format_double(10000.0 / res));
  }
  g_clear_error(&tmp_err);
}

static void ndpi_set_string_prop(openremoteslide_t *osr,
                                 struct _openremoteslide_tifflike *tl,
                                 int64_t dir, int32_t tag,
                                 const char *property_name) {
  const char *value = _openremoteslide_tifflike_get_buffer(tl, dir, tag, NULL);
  if (value) {
    g_hash_table_insert(osr->properties,
                        g_strdup(property_name),
                        g_strdup(value));
...
static void ndpi_set_props(openremoteslide_t *osr,
                           struct _openremoteslide_tifflike *tl, int64_t dir) {
  // MPP
  ...
  // ASCII property map
  const char *props = _openremoteslide_tifflike_get_buffer(tl, dir,
...
static bool hamamatsu_ndpi_open(openremoteslide_t *osr, const char *filename,
                                struct _openremoteslide_tifflike *tl,
                                struct _openremoteslide_hash *quickhash1,
                                GError **err) {
  ...
  // walk directories
  int64_t directories = _openremoteslide_tifflike_get_directory_count(tl);
  ...
  for (int64_t dir = 0; dir < directories; dir++) {
    ...
    start_in_file = _openremoteslide_tifflike_uint_fix_offset_ndpi(tl, dir,
                                                             start_in_file);

    double lens =
      _openremoteslide_tifflike_get_float(tl, dir, NDPI_SOURCELENS, &tmp_err);
    if (tmp_err) {
      g_propagate_error(err, tmp_err);
      goto FAIL;
    }
    ...
    if (lens > 0) {
      ...
      int64_t focal_plane =
        _openremoteslide_tifflike_get_sint(tl, dir, NDPI_FOCAL_PLANE, &tmp_err);
      if (tmp_err) {
        g_propagate_error(err, tmp_err);
        goto FAIL;
      }
      ...
      if (urlio_fseek(f, start_in_file, SEEK_SET)) {
        _openremoteslide_io_error(err, "Couldn't seek to JPEG start");
        goto FAIL;
      }
      if (!validate_jpeg_header(f, dimensions_valid,
                                ...
                                NULL, &tmp_err)) {
        if (g_error_matches(tmp_err, OPENREMOTESLIDE_HAMAMATSU_ERROR,
                            OPENREMOTESLIDE_HAMAMATSU_ERROR_NO_RESTART_MARKERS)) {
          // non-tiled image
      ...
      if (jp->tile_count > 1) {
        int64_t mcu_start_count =
          _openremoteslide_tifflike_get_value_count(tl, dir, NDPI_MCU_STARTS);

        if (mcu_start_count == jp->tile_count) {
          //g_debug("loading MCU starts for directory %"PRId64, dir);
          const uint64_t *unreliable_mcu_starts =
            _openremoteslide_tifflike_get_uints(tl, dir, NDPI_MCU_STARTS, NULL);
          if (unreliable_mcu_starts) {
        ...
        if (jp->unreliable_mcu_starts == NULL) {
          ...
          _openremoteslide_performance_warn("Bad or missing MCU starts for "
      ...
      // macro image
      if (!_openremoteslide_jpeg_add_associated_image(osr, "macro",
  ...
  if (!_openremoteslide_tifflike_init_properties_and_hash(osr, tl, quickhash1,
                                                    min_width_dir, 0,
                                                    err)) {
    goto FAIL;
  }
...
const struct _openremoteslide_format _openremoteslide_format_hamamatsu_ndpi = {
  .name = "hamamatsu-ndpi",
  .vendor = "hamamatsu",
