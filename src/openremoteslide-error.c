...
void _openremoteslide_propagate_error(openremoteslide_t *osr, GError *err) {
...
GQuark _openremoteslide_error_quark(void) {
  return g_quark_from_string("openremoteslide-error-quark");
}

void _openremoteslide_io_error(GError **err, const char *fmt, ...) {
  int my_errno = errno;
  va_list ap;
...
bool _openremoteslide_check_cairo_status(cairo_t *cr, GError **err) {
  ...
  g_set_error(err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_CAIRO_ERROR,
