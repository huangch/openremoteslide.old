...
static void make_ycbcr_tables(FILE *f) {
  // R
  fprintf(f, "const int16_t _openremoteslide_R_Cr[256] = {");
  for (int i = 0; i < 256; i++) {
    if (!(i % 10)) {
      fprintf(f, "\n ");
    }
  ...
  fprintf(f, "const int32_t _openremoteslide_G_Cb[256] = {");
  for (int i = 0; i < 256; i++) {
    if (!(i % 5)) {
      fprintf(f, "\n ");
    }
  ...
  fprintf(f, "\n};\n\n");
  fprintf(f, "const int32_t _openremoteslide_G_Cr[256] = {");
  for (int i = 0; i < 256; i++) {
    if (!(i % 5)) {
      fprintf(f, "\n ");
    }
  ...
  // B
  fprintf(f, "const int16_t _openremoteslide_B_Cb[256] = {");
  for (int i = 0; i < 256; i++) {
    if (!(i % 10)) {
      fprintf(f, "\n ");
    }
...
int main(int argc, char **argv) {
  ...
  fprintf(f, "#include \"openremoteslide-private.h\"\n\n");
