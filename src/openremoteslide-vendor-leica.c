 ...
 *
 * quickhash comes from _openremoteslide_tifflike_init_properties_and_hash
 *
 */
...
#define PARSE_INT_ATTRIBUTE_OR_FAIL(NODE, NAME, OUT)		\
  do {								\
    GError *tmp_err = NULL;					\
    OUT = _openremoteslide_xml_parse_int_attr(NODE, NAME, &tmp_err);	\
    if (tmp_err)  {						\
...
static void destroy_level(struct level *l) {
  for (uint32_t n = 0; n < l->areas->len; n++) {
    ...
    _openremoteslide_grid_destroy(area->grid);
...
static void destroy(openremoteslide_t *osr) {
  struct leica_ops_data *data = osr->data;
  _openremoteslide_tiffcache_destroy(data->tc);
...
static bool read_tile(openremoteslide_t *osr,
                      cairo_t *cr,
                      struct _openremoteslide_level *level G_GNUC_UNUSED,
                      int64_t tile_col, int64_t tile_row,
                      void *arg,
                      GError **err) {
  struct read_tile_args *args = arg;
  struct _openremoteslide_tiff_level *tiffl = &args->area->tiffl;
  ...
  if (!tiledata) {
    ...
    if (!_openremoteslide_tiff_read_tile(tiffl, args->tiff,
    ...
    // clip, if necessary
    if (!_openremoteslide_tiff_clip_tile(tiffl, tiledata,
                                   tile_col, tile_row,
                                   err)) {
      g_slice_free1(tw * th * 4, tiledata);
      return false;
...
static bool paint_region(openremoteslide_t *osr, cairo_t *cr,
			 int64_t x, int64_t y,
  ...
  TIFF *tiff = _openremoteslide_tiffcache_get(data->tc, err);
  if (tiff == NULL) {
    return false;
  }
  ...
  for (uint32_t n = 0; n < l->areas->len; n++) {
    ...
    success = _openremoteslide_grid_paint_region(area->grid, cr, &args,
                                           ax, ay, level, w, h,
                                           err);
    if (!success) {
      break;
  ...
  _openremoteslide_tiffcache_put(data->tc, tiff);
  return success;
}

static const struct _openremoteslide_ops leica_ops = {
  .paint_region = paint_region,
  .destroy = destroy,
};

static bool leica_detect(const char *filename G_GNUC_UNUSED,
                         struct _openremoteslide_tifflike *tl, GError **err) {
  ...
  if (!_openremoteslide_tifflike_is_tiled(tl, 0)) {
    g_set_error(err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_FAILED,
                "TIFF is not tiled");
    return false;
  }
  ...
  const char *image_desc = _openremoteslide_tifflike_get_buffer(tl, 0,
  ...
  xmlDoc *doc = _openremoteslide_xml_parse(image_desc, err);
  if (doc == NULL) {
    return false;
  }

  // check default namespace
  if (!_openremoteslide_xml_has_default_namespace(doc, LEICA_XMLNS_1) &&
      !_openremoteslide_xml_has_default_namespace(doc, LEICA_XMLNS_2)) {
    g_set_error(err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_FAILED,
...
static void set_resolution_prop(openremoteslide_t *osr, TIFF *tiff,
  ...
  if (TIFFGetFieldDefaulted(tiff, TIFFTAG_RESOLUTIONUNIT, &unit) &&
    ...
    g_hash_table_insert(osr->properties, g_strdup(property_name),
                        _openremoteslide_format_double(10000.0 / f));
  }
}

static void set_region_bounds_props(openremoteslide_t *osr,
                                    struct level *level0) {
  ...
  for (uint32_t n = 0; n < level0->areas->len; n++) {
    ...
    g_hash_table_insert(osr->properties,
                        g_strdup_printf(_OPENREMOTESLIDE_PROPERTY_NAME_TEMPLATE_REGION_X, n),
                        g_strdup_printf("%"PRId64, area->offset_x));
    g_hash_table_insert(osr->properties,
                        g_strdup_printf(_OPENREMOTESLIDE_PROPERTY_NAME_TEMPLATE_REGION_Y, n),
                        g_strdup_printf("%"PRId64, area->offset_y));
    g_hash_table_insert(osr->properties,
                        g_strdup_printf(_OPENREMOTESLIDE_PROPERTY_NAME_TEMPLATE_REGION_WIDTH, n),
                        g_strdup_printf("%"PRId64, area->tiffl.image_w));
    g_hash_table_insert(osr->properties,
                        g_strdup_printf(_OPENREMOTESLIDE_PROPERTY_NAME_TEMPLATE_REGION_HEIGHT, n),
...
static struct collection *parse_xml_description(const char *xml,
  ...
  // parse the xml
  xmlDoc *doc = _openremoteslide_xml_parse(xml, err);
  if (doc == NULL) {
    return false;
  }

  // create XPATH context to query the document
  ctx = _openremoteslide_xml_xpath_create(doc);
  ...
  */
  ...
  // get collection node
  xmlNode *collection_node = _openremoteslide_xml_xpath_get_node(ctx,
                                                           "/d:scn/d:collection");
  if (!collection_node) {
    g_set_error(err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_FAILED,
  ...
  char *barcode = _openremoteslide_xml_xpath_get_string(ctx, "/d:scn/d:collection/d:barcode/text()");
  if (barcode) {
    // Decode Base64
    gsize len;
    ...
    collection->barcode = _openremoteslide_xml_xpath_get_string(ctx, "/d:scn/d:collection/@barcode");
  }
  ...
  images_result = _openremoteslide_xml_xpath_eval(ctx, "d:image");
  if (!images_result) {
    g_set_error(err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_FAILED,
  ...
  for (int i = 0; i < images_result->nodesetval->nodeNr; i++) {
    ...
    // get view node
    xmlNode *view = _openremoteslide_xml_xpath_get_node(ctx, "d:view");
    if (!view) {
      g_set_error(err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_FAILED,
    ...
    image->creation_date = _openremoteslide_xml_xpath_get_string(ctx, "d:creationDate/text()");
    image->device_model = _openremoteslide_xml_xpath_get_string(ctx, "d:device/@model");
    image->device_version = _openremoteslide_xml_xpath_get_string(ctx, "d:device/@version");
    image->illumination_source = _openremoteslide_xml_xpath_get_string(ctx, "d:scanSettings/d:illuminationSettings/d:illuminationSource/text()");
    image->objective = _openremoteslide_xml_xpath_get_string(ctx, "d:scanSettings/d:objectiveSettings/d:objective/text()");
    image->aperture = _openremoteslide_xml_xpath_get_string(ctx, "d:scanSettings/d:illuminationSettings/d:numericalAperture/text()");
    ...
    // get dimensions
    ctx->node = image_node;
    result = _openremoteslide_xml_xpath_eval(ctx, "d:pixels/d:dimension");
    if (!result) {
      g_set_error(err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_FAILED,
...
static void set_prop(openremoteslide_t *osr, const char *name, const char *value) {
  if (value) {
    g_hash_table_insert(osr->properties,
                        g_strdup(name),
                        g_strdup(value));
...
static bool create_levels_from_collection(openremoteslide_t *osr,
                                          struct _openremoteslide_tiffcache *tc,
                                          TIFF *tiff,
  ...
  *quickhash_dir = -1;
  ...
  for (uint32_t image_num = 0; image_num < collection->images->len;
    ...
    if (!first_main_image) {
      ...
      _openremoteslide_duplicate_int_prop(osr, "leica.objective",
    ...
    for (uint32_t dimension_num = 0; dimension_num < image->dimensions->len;
      ...
      struct _openremoteslide_tiff_level *tiffl = &area->tiffl;
      g_ptr_array_add(l->areas, area);

      // select and examine TIFF directory
      if (!_openremoteslide_tiff_level_init(tiff, dimension->dir,
                                      NULL, tiffl,
                                      err)) {
        return false;
      }
      ...
      // create grid
      area->grid = _openremoteslide_grid_create_simple(osr,
                                                 tiffl->tiles_across,
                                                 tiffl->tiles_down,
                                                 tiffl->tile_w,
                                                 tiffl->tile_h,
  ...
  for (uint32_t image_num = 0; image_num < collection->images->len;
    ...
    if (!_openremoteslide_tiff_add_associated_image(osr, "macro", tc,
                                              dimension->dir, err)) {
      return false;
    }
...
static bool leica_open(openremoteslide_t *osr, const char *filename,
  ...
  // open TIFF
  struct _openremoteslide_tiffcache *tc = _openremoteslide_tiffcache_create(filename);
  ...
  g_hash_table_remove(osr->properties, OPENREMOTESLIDE_PROPERTY_NAME_COMMENT);
  g_hash_table_remove(osr->properties, "tiff.ImageDescription");

  // set MPP properties
  if (!_openremoteslide_tiff_set_dir(tiff, property_dir, err)) {
    goto FAIL;
  }
  set_resolution_prop(osr, tiff, OPENREMOTESLIDE_PROPERTY_NAME_MPP_X,
                      TIFFTAG_XRESOLUTION);
  set_resolution_prop(osr, tiff, OPENREMOTESLIDE_PROPERTY_NAME_MPP_Y,
                      TIFFTAG_YRESOLUTION);
...
const struct _openremoteslide_format _openremoteslide_format_leica = {
  .name = "leica",
  .vendor = "leica",
  .detect = leica_detect,
  .open = leica_open,
