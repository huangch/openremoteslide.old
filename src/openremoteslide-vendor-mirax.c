...
#define POSITIVE_OR_FAIL(N)					\
  do {								\
    if (N <= 0) {						\
      g_set_error(err, OPENREMOTESLIDE_ERROR,				\
                  OPENREMOTESLIDE_ERROR_FAILED, #N " <= 0: %d", N);	\
      goto FAIL;						\
    }								\
  } while(0)
...
// see comments in _openremoteslide_try_mirax()
...
static uint32_t *read_image(openremoteslide_t *osr,
                            struct image *image,
  ...
  switch (format) {
  case FORMAT_JPEG:
    result = _openremoteslide_jpeg_read(data->datafile_paths[image->fileno],
                                  image->start_in_file,
                                  dest, w, h,
                                  err);
    break;
  case FORMAT_PNG:
    result = _openremoteslide_png_read(data->datafile_paths[image->fileno],
                                 image->start_in_file,
                                 dest, w, h,
                                 err);
    break;
  case FORMAT_BMP:
    result = _openremoteslide_gdkpixbuf_read("bmp",
...
static bool paint_region(openremoteslide_t *osr G_GNUC_UNUSED, cairo_t *cr,
                         int64_t x, int64_t y,
                         struct _openremoteslide_level *level,
                         int32_t w, int32_t h,
                         GError **err) {
...
static const struct _openremoteslide_ops mirax_ops = {
  .paint_region = paint_region,
  .destroy = destroy,
};

static bool mirax_detect(const char *filename, struct _openremoteslide_tifflike *tl,
                         GError **err) {
  // reject TIFFs
  if (tl) {
    g_set_error(err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_FAILED,
...
static bool read_le_int32_from_file_with_result(URLIO_FILE *f, int32_t *_OUT) {
  if (urlio_fread(_OUT, 4, 1, f) != 1) {
    return false;
  }

  *_OUT = GINT32_FROM_LE(*_OUT);
  //  g_debug("%d", i);
...
static bool read_nonhier_record(URLIO_FILE *f,
  ...
  // http://lists.andrew.cmu.edu/pipermail/openremoteslide-users/2013-August/000634.html
...
static void insert_tile(struct level *l,
  ...
  // insert
  _openremoteslide_grid_tilemap_add_tile(l->grid,
                                   tile_x, tile_y,
                                   offset_x, offset_y,
                                   l->tile_w, l->tile_h,
                                   tile);
...
static bool process_hier_data_pages_from_indexfile(URLIO_FILE *f,
  ...
  for (int zoom_level = 0; zoom_level < zoom_levels; zoom_level++) {
	...
	if (zoom_level == zoom_levels - 1) {
	  if (!_openremoteslide_hash_file_part(quickhash1, datafile_paths[fileno],
	                                 offset, length, err)) {
...
static void *read_record_data(const char *path,
                              ...
                              GError **err) {
  void *buffer = NULL;
  URLIO_FILE *f = _openremoteslide_fopen(path, "rb", err);
  if (!f) {
    return NULL;
  }
...
static bool add_associated_image(openremoteslide_t *osr,
  ...
  return _openremoteslide_jpeg_add_associated_image(osr, name, path, offset, err);
}


static bool process_indexfile(openremoteslide_t *osr,
			      const char *uuid,
			      int datafile_count,
			      char **datafile_paths,
...
static void add_properties(openremoteslide_t *osr, GKeyFile *kf) {
...
static bool mirax_open(openremoteslide_t *osr, const char *filename,
  ...
  // hash the slidedat
  if (!_openremoteslide_hash_file(quickhash1, tmp, err)) {
    goto FAIL;
  }

  slidedat = g_key_file_new();
  if (!_openremoteslide_read_key_file(slidedat, tmp, SLIDEDAT_MAX_SIZE,
                                G_KEY_FILE_NONE, err)) {
  ...
  */
  ...
  indexfile = _openremoteslide_fopen(tmp, "rb", err);
  g_free(tmp);
  tmp = NULL;
  ...
  for (int i = 0; i < zoom_levels; i++) {
    ...
    // create grid
    l->grid = _openremoteslide_grid_create_tilemap(osr,
                                             lp->tile_advance_x,
                                             lp->tile_advance_y,
                                             read_tile, tile_free);
  ...
  // set properties
  _openremoteslide_set_bounds_props_from_grid(osr, levels[0]->grid);
  uint32_t fill = slide_zoom_level_sections[0].fill_rgb;
  _openremoteslide_set_background_color_prop(osr,
                                       (fill >> 16) & 0xFF,
                                       (fill >> 8) & 0xFF,
                                       fill & 0xFF);
  g_hash_table_insert(osr->properties,
                      g_strdup(OPENREMOTESLIDE_PROPERTY_NAME_OBJECTIVE_POWER),
                      g_strdup_printf("%d", objective_magnification));
  g_hash_table_insert(osr->properties,
                      g_strdup(OPENREMOTESLIDE_PROPERTY_NAME_MPP_X),
                      _openremoteslide_format_double(slide_zoom_level_sections[0].mpp_x));
  g_hash_table_insert(osr->properties,
                      g_strdup(OPENREMOTESLIDE_PROPERTY_NAME_MPP_Y),
                      _openremoteslide_format_double(slide_zoom_level_sections[0].mpp_y));
...
const struct _openremoteslide_format _openremoteslide_format_mirax = {
  .name = "mirax",
  .vendor = "mirax",
  .detect = mirax_detect,
  .open = mirax_open,
