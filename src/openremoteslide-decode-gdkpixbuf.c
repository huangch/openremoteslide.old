...
static void area_prepared(GdkPixbufLoader *loader, void *data) {
  ...
  if (gdk_pixbuf_get_colorspace(pixbuf) != GDK_COLORSPACE_RGB ||
    ...
    g_set_error(&state->err, OPENREMOTESLIDE_ERROR, OPENREMOTESLIDE_ERROR_FAILED,
...
bool _openremoteslide_gdkpixbuf_read(const char *format,
                               const char *filename,
                               int64_t offset,
                               int64_t length,
                               uint32_t *dest,
  ...
  if (urlio_fseek(f, offset, SEEK_SET)) {
    _openremoteslide_io_error(err, "Couldn't urlio_fseek %s", filename);
    goto DONE;
  }
